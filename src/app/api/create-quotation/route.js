...
export async function POST(request) {
  try {
    ...
    if (
      !slidingWindowData ||
      !Array.isArray(slidingWindowData) ||
      !quotationFormData
    ) {
    ...
    doc.text("ใบเสนอราคาเบื้องต้น (Drafted Quotation)", 105, 45, {
      align: "center",
    }); // Centered Title (Thai)
    doc.setFontSize(10);
    ...
    doc.text(
      `${quotationFormData.place || "-"}`,
      quotationDetailsX + 20,
      quotationDetailsY
    );
    quotationDetailsY += 5;
    doc.text(`Project`, quotationDetailsX, quotationDetailsY);
    doc.text(
      `${quotationFormData.project || "-"}`,
      quotationDetailsX + 20,
      quotationDetailsY
    );
    quotationDetailsY += 5;
    ...
    doc.text(
      `${quotationFormData.quote || "-"}`,
      quotationDetailsX + 20,
      quotationDetailsY + 5
    ); // Quoted by X and Y adjusted
    ...
    const addTableCell = (
      ...
      if (isPrice && typeof text === "number") {
        // **If isPrice and text is a number, format it**
        displayValue = text.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        });
      }
      doc.text(displayValue, x + width / 2, y + height / 2 + 2, {
        align: align,
    ...
    slidingWindowData.forEach((item) => {
        console.log("multilock: "+item.ml);
        let lockType=""
        if(item.ml==true&&item.sl==false){
            lockType = "Multi-lock";
        }
        if(item.ml==false&&item.sl==true){
            lockType = "Single-lock";
        }else{
            lockType = "";}
      currentX = tableX;
      const totalPrice =
        item.price && item.qty ? Number(item.price) * Number(item.qty) : 0; // Ensure totalPrice is a number
      const rowData = [
        /* item.id, */
        item.id,
        "SL5000",
      ...
      rowData.forEach((cell, index) => {
        ...
        addTableCell(
          ...
          cellAlign,
          isPriceCell
        ); // **Pass isPriceCell to addTableCell**
        currentX += cellWidths[index];
      });
      tableY += 10;
    });
    ...
    let subtotal = slidingWindowData.reduce(
    ...
    let vatAmount = 0; // VAT on discounted subtotal
    ...
    if (discountPercent > 0) {
      // Conditionally display % discount if > 0
      doc.text(`ส่วนลด (${discountPercent}%)`, summaryX, summaryY, {
        align: "right",
      }); // Discount % Label
      doc.text(
    ...
    if (discountAmount > 0) {
      // Conditionally display fixed discount if > 0
    ...
    if (installationCost > 0) {
      ...
      doc.text(
        `${installationCost.toLocaleString("en-US", {
          minimumFractionDigits: 2,
          maximumFractionDigits: 2,
        })}`,
        summaryAmountX,
    ...
    doc.text("Kunnapab Home Solution Ltd. (Head Office)", 195, footerY + 5, {
      align: "right",
    }); // Bottom Right
    ...
    return new Response(
      JSON.stringify({ error: "Failed to generate PDF with jsPDF" }),
      {
        status: 500,
        headers: { "Content-Type": "application/json" },
      }
